...
#     """
#     Convert an Intent object into a plain dictionary that pandas_runner can use.
#     Handles task, measures, dims, filters, and time_range (YTD/MAT).
#     """
...
#     dims = intent.dims or []
#     if task == "topn":
#         # For Top-N, drop 'date' so we rank across whole window
...
#     # ---- measures ----
#     measures = intent.measures or ["value_sales"]

#     # ✅ If both appear, force unit_sales to be the primary measure
#     if "unit_sales" in measures:
#         measures = ["unit_sales"]
#     elif "value_sales" in measures:
#         measures = ["value_sales"]
...
#         "time_range": tr,
#         "top_n": (intent.sort.limit if (intent.sort and intent.sort.limit) else None),
...
def parse_user_text(text: str) -> Intent:
    ...
    # If both unit_sales and value_sales were picked, prefer unit_sales
    ...
    if any(x in t for x in ["brand", "brands"]):
        if "brand" not in intent.dims:
            intent.dims.append("brand")
    if any(x in t for x in ["category", "segment"]):
        if "category" not in intent.dims:
            intent.dims.append("category")
    if "market" in t or "country" in t:
        if "market" not in intent.dims:
            intent.dims.append("market")
    if "channel" in t:
        if "channel" not in intent.dims:
            intent.dims.append("channel")
    ...
    # If YoY mentioned, set sort key when ranking
    if "yoy" in t:
        if intent.sort is None:
            intent.sort = SortSpec()
        if intent.sort.by is None:
    ...
    # --- time ranges mentioned as literal "last 12" (kept for backward compatibility) ---
    if "last 12" in t or "last twelve" in t:
        y_end, m_end = _last_complete_month_anchor()
        start_idx = (y_end * 12 + (m_end - 1)) - 11
        start_y, start_m = divmod(start_idx, 12)
...
def intent_to_dict(intent: Intent, original_text: str = "") -> dict:
    ...
    Convert an Intent object into a plain dictionary that pandas_runner can use.
    Handles task, measures, dims, filters, and time_range (YTD/MAT).
    - Top-N: if user asked "Top N", we coerce task to "topn" and drop 'date' from dims.
    - Time range: infer "MAT" / "YTD" directly from the raw text if present.
    - Measures: preserve what parser picked; prefer unit_sales over value_sales when both detected.
    """
    t = (original_text or "").lower()
    ...
    dims = intent.dims or []
    if task == "topn":
        dims = [d for d in dims if d != "date"]  # rank over the whole window
